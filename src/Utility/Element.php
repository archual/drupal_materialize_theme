...
/**
 * @file
 * Contains \Drupal\materialize\Utility\Element.
 */
...
class Element extends DrupalAttributes {
  ...
  public function colorize() {
    ...
    if ($button && !Materialize::getTheme()->getSetting('button_colorize')) {
      $this->addClass('btn-default');
      return $this;
    }

    if ($value = $this->getProperty('value', $this->getProperty('title'))) {
      $class = "$prefix-" . Materialize::cssClassFromString($value, $this->getProperty('button_type', 'default'));
      $this->addClass($class);
      if ($button && $this->getProperty('split')) {
        $this->addClass($class, $this::SPLIT_BUTTON);
      }
  ...
  public function setButtonSize($size = NULL) {
    ...
    if ($size = $size ?: Materialize::getTheme()->getSetting('button_size')) {
      $this->addClass($size);
      if ($this->getProperty('split')) {
